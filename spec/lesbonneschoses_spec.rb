...
describe 'LesBonnesChoses' do
	...
	describe 'API::Document' do
		before do
			@document = @api.form('everything').query(%([[:d = at(document.id, "UkL0gMuvzYUANCpf")]])).submit(@master_ref).results[0]
		end
